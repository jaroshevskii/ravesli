...
Напишите класс, который реалызует функционал <!-- FIX: Empty link --> [стека](). Класс Stack должен иметь:

- закрытый целочисленый <!-- FIX: Empty link --> [фиксированый массив]() длиной 10 элементов;
...
- открытый метод с именем pop() для возврата значений из стека. Если в стеке нет значений, то должен выводится <!-- FIX: Empty link --> [сейтмент assert]();
